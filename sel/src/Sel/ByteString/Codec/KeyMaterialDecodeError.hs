...
-- during decoding.
module Sel.ByteString.Codec.KeyMaterialDecodeError
  ( KeyMaterialDecodeError (..) -- ^ @since 0.0.3.0
  , RequiredLength (..) -- ^ @since 0.0.3.0
  , InputLength (..) -- ^ @since 0.0.3.0
  , validKeyMaterialHexBytes -- ^ @since 0.0.3.0
  , validKeyMaterialLength -- ^ @since 0.0.3.0
  ) where
