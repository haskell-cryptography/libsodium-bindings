...
module Sel.ByteString.Codec.KeyPointer
  ( -- * Key material pointer utilities
    KeyPointerSize (..) -- ^ @since 0.0.3.0
  , keyPointer -- ^ @since 0.0.3.0
  , keyPointerLength -- ^ @since 0.0.3.0

    -- ** Deriving instances
  , KeyPointer (..) -- ^ @since 0.0.3.0
  , type KeyCoerce -- ^ @since 0.0.3.0
  , type ComparisonImplementation (..) -- ^ @since 0.0.3.0
  )
where
...
--   deriving (Eq, Ord, HexEncode) via (KeyPointer SomeKeyMaterial ShortCircuiting)
--
