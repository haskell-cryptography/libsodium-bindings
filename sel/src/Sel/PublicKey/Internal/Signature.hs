...
module Sel.PublicKey.Internal.Signature
  ...
  , public -- ^ @since 0.0.3.0
  , secret -- ^ @since 0.0.3.0
  , keyPair -- ^ @since 0.0.3.0
  ...
  , extractUnverifiedMessage -- ^ @since 0.0.3.0
  , extractSignature -- ^ @since 0.0.3.0
  , buildSignedMessage -- ^ @since 0.0.3.0
...
import Sel.Internal.Constraints
  ( illegal
  , type (:!!!:)
  , type (:<:)
  )
...
import Sel.Internal.Scoped.FFI (memcpy)
...
-- | Decode bytes to a t'PublicKey'.
--
-- The input is checked for:
--
-- * Hexadecimal encoding, yielding @'Left' ('Sel.ByteString.Codec.KeyMaterialDecodeError.DecodingFailure' message)@ on failure
-- * Length matching the pointer size, yielding @'Left' ('Sel.ByteString.Codec.KeyMaterialDecodeError.ByteLengthMismatch' requiredLength inputLength)@ on failure
--
-- @since 0.0.3.0
instance HexDecode PublicKey where
  decodeHexBytes = toKey . Base16.decodeBase16
...
-- | By lexicographical comparison of pointer contents.
--
-- This instance is vulnerable to timing attacks and has been made illegal.
--
-- Use the t'UnsafeSecretKey' newtype if you need this.
--
-- @since 0.0.3.0
instance SecretKey :<: UnsafeSecretKey => Ord SecretKey where
  compare = illegal
...
--
-- @since 0.0.3.0
instance HexDecode SecretKey where
  decodeHexBytes = toKey . Base16.decodeBase16

-- | A secret key should not be transmitted without care. Use the
-- t'UnsafeSecretKey' newtype to signal intent.
--
-- @since 0.0.3.0
instance SecretKey :!!!: UnsafeSecretKey => HexEncode SecretKey where
  encodeHexBytes = illegal
...
-- @since 0.0.3.0
deriving via
  (KeyPointer SecretKey ConstantTime)
  instance
    {-# WARNING in "x-sel-timing-attack"
      [ "This instance is vulnerable to timing attacks."
      , "Use -Wno-x-sel-timing-attack to silence this warning."
      ]
      #-}
    Ord UnsafeSecretKey
...
unverifiedExtract target fieldLength (target -> field) = do
  ...
  memcpy bsPtr fieldPtr fieldLength
  lift $ do
